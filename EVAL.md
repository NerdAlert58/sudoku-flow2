...
| Contract edge | Every route enforces: 415 wrong content type, 413 both caps, 400 malformed JSON, 405 wrong method w/ envelope + Allow, 404 unknown /v1 path, 500 panic recovery, security headers on every response, no ACAO header ever, solve-shape 400 for invalid puzzles | Table-driven handler tests per route × per edge case | Synthetic requests (incl. `.`-blank puzzles, duplicate givens, bad lengths) | Every cell of the route×edge matrix passes | Any envelope code, HTTP status, or header deviation |
...
- **Synthetic invalid/edge inputs** (table-driven, inline in tests): bad lengths, bad
  ...
  D1), CRLF/whitespace batch lines, malformed batch entries, over-cap bodies and lists.
