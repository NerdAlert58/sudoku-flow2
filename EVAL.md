...
| UC-4 Batch | Full-corpus batch returns 55 in-order results, `solvedCount:55`; malformed lines are per-item failures; caps enforced 413-before-solve; race-clean | Handler test with all 55 + synthetic batches (CRLF/whitespace lines, malformed lines, 257 items, >1 MiB) under `-race` | `puzzles.txt` + synthetic batch fixtures | All assertions pass under `-race` | Any ordering, aggregation, cap, or race failure, or any batch-item field value deviating from ADR-0014 |
